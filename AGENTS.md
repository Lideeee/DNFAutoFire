...
## 命令执行
- 当前仓库默认使用 PowerShell，不要使用 Bash heredoc 形式，例如 `apply_patch <<'PATCH'`
- 不要把多个 heredoc 或多个补丁塞进同一个 PowerShell 命令
- 不要依赖 `@'...'@ | apply_patch` 管道方式
- 批量新建或移动文件时使用 PowerShell 原生命令；修改已有文件时优先单独调用 `apply_patch`
## 新增 EX 功能指引

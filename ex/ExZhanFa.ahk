; 战法：触发键热键驱动；任一监听键按住时按主间隔持续输出，全部抬起后释放。
...
class ExZhanFa {
    ...
    static Run() {
        ...
        triggerKeys := ZhanFaUniqueSkillKeysByPressKey(ZhanFaLoadKeys(presetName))
        ...
        for keyName in triggerKeys {
            ...
            entries[scID] := {
                ...
                probeKey: probeKey,
                isHeld: false,
                downFn: ObjBindMethod(ExZhanFa, "Down", scID),
                upFn: ObjBindMethod(ExZhanFa, "Up", scID)
            }
        }
        if (entries.Count = 0) {
            return
        ...
        this._ctx := {
            ...
            timerFn: ObjBindMethod(ExZhanFa, "Tick")
        }
        OnExit(ObjBindMethod(ExZhanFa, "OnExit"))
        this._EnableHooks()
        Suspend(false)
        loop {

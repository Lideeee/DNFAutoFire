...
UiLayoutRect(layout, x, y, w, h, extra := "") {
    if IsObject(layout) {
        return layout.Rect(x, y, w, h, extra)
    }
    return UiRect(x, y, w, h, extra)
}

UiLayoutTrack(layout, x, y, w, h) {
    if IsObject(layout) {
        layout.Track(x, y, w, h)
    }
}

class UiContentLayout {
    __New(padRight := 16, padBottom := 16) {
    ...
    Width(minWidth := 0) {
        width := this.maxRight + this.padRight
        return width > minWidth ? width : minWidth
    }

    Height(minHeight := 0) {
        height := this.maxBottom + this.padBottom
...
class MainKeyLayoutData {
    ...
    static AlphaBlockRight() {
        ...
        x += 9 * this.Pitch()
        x += 2 * this.Pitch()
        x += this.KeyWidth(2.25)
        return x
    }

    static NumpadX() {
        return this.AlphaBlockRight() + this.Gap()
    }
